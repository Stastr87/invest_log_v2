...
log_operations_service = logging.getLogger(__name__)
if logger_level.lower() == 'debug':
    log_operations_service.setLevel(logging.DEBUG)
if logger_level.lower() == 'error':
    log_operations_service.setLevel(logging.ERROR)
if logger_level.lower() == 'info':
    log_operations_service.setLevel(logging.INFO)
if logger_level.lower() == 'warning':
    log_operations_service.setLevel(logging.WARNING)
if logger_level.lower() == 'critical':
    log_operations_service.setLevel(logging.CRITICAL)
if logger_level == None:
    log_operations_service.setLevel(logging.NOTSET)
...
log_operations_service.addHandler(fh)
...
class Operations:
    ...
    def test_func(self):
        # logger.info(f'{self.__class__.__name__} I am OK!!!')
        return f'{self.__class__.__name__} I am OK!!!'
    ...
    def get_my_positions_by_account(self):
        ...
        response_data_list = []
        for account_id_item in account_id_list:
            log_operations_service.debug(f'{__name__} -> get_my_positions_by_account() -> account_id_item:{account_id_item}')
            # account_id = account_id_item
            with Client(self.token) as client:
                data = client.operations.get_positions(account_id=account_id_item)
                log_operations_service.debug(f'{__name__} -> get_my_positions_by_account() -> data:{data}')
                positions_by_account={"account_id":account_id_item,
                                      "positions":data}
